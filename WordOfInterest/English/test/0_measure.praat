...
# This script measures various acoustic variables 
# in all soundfiles in a particular directory
# for each interval annotated in a particular tier in a praat textgrid
# it will create a line in a tab-delimited spread sheet with the acoustic measures
# for certain analyses i use the R  reshape package to reorganize the spread sheet 
# into a format where each line corresponds to an utterance, and you get multiple sets of columns
# with the acoustic measures for each word on a single line
...
form Calculate Results for Production Experiments
	# the soundfile name can include information separated by underscores
	# This field tells praat what the column names should be for that information:
	sentence Name_Format experiment_subject_item_condition
	sentence Result_filename acousticMeasures
	sentence extension .wav
	comment if left empty, then all sound files script directory:
	sentence Sound_directory
	# by default, this script saves a .csv file:
	sentence seperator ,
	natural wordTier 1
	comment Is there a separate tier indicating which are the words of interest?
	boolean woi_only no
	natural woiTier 1
endform
